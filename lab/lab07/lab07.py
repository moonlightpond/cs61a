...
class FreeChecking(Account):
    ...
    def withdraw(self, amount):
        ...
        if  amount + (0 if self.free_withdrawals >= 0 else self.withdraw_fee) > self.balance:
            return "Insufficient funds"
        if amount > self.max_withdrawal:
            return "Can't withdraw that amount"
        self.balance = self.balance - amount - (0 if self.free_withdrawals >= 0 else self.withdraw_fee)
        return self.balance
...
def without(s, i):
    ...
    """
    def h(ss, ii, index):
        if index < ii and ss.rest is not Link.empty:
            return Link(ss.first, h(ss.rest, ii, index + 1))
        elif index == ii:
            return ss.rest
        elif ss.rest is Link.empty:
            return ss
    return h(s, i, 0)
    """
    if s is Link.empty:
